...
Original file is located at
    https://colab.research.google.com/drive/1VRKrFP6yXsIJWecO7g2SEj9C0n6p17te
"""
...
# initial_state_phi1=None
# initial_state_0_phi1=None
...
def layer_t3_no_HT(circuit, theta, offset, num_of_qubits, qubit_posi):
    ...
    if(offset):
        ...
        for i in range(num_of_qubits-1):
            circuit.ry(qubit_posi[i], theta=theta[i])
        for i in np.arange(0, num_of_qubits-2, 2):
            circuit.CNOT(qubit_posi[i], qubit_posi[i+1])  # CNOT struct3
        for i in range(num_of_qubits-1):
            circuit.ry(qubit_posi[i], theta=theta[i+num_of_qubits-1])
        for i in np.arange(1, num_of_qubits-2, 2):
            circuit.CNOT(qubit_posi[i], qubit_posi[i+1])  # CNOT struct3
        circuit.CNOT(qubit_posi[-1], qubit_posi[0])  # CNOT struct3
...
def quantum_circuit_no_Z(num_of_qubits, theta, start_state=None):
    ...
    circuit = tc.Circuit(num_of_qubits, inputs=initial_state_0_phi1)
...
def quantum_circuit_no_HT_return_state(num_of_qubits, theta, start_state=None):
    # initiate state vector |phi_1>
    circuit = tc.Circuit(num_of_qubits-1, inputs=initial_state_phi1)
    # circ_phi.initialize(start_state, circ_phi.qubits)
    ...
    for theta_i in theta:
        layer_t3_no_HT(circuit=circuit, theta=theta_i,
                       num_of_qubits=num_of_qubits, qubit_posi=list(range(num_of_qubits-1)))
...
for rep in range(1, max_repeat+1):
    ...
    result = quantum_circuit_no_HT_return_state(
        theta=theta, num_of_qubits=num_of_qubits, start_state=initial_state_phi1)
    state_last = result[-1]
    ...
    # shape1 = (num_of_layers, 2*(num_of_qubits))
    # f_scipy = tc.interfaces.scipy_interface(objective_fn, shape=shape1, jit=True)
    # params = K.implicit_randn(shape1)
    # print(params)
    ...
    # shape1 = (num_of_layers, 2*(num_of_qubits))
    # params = K.implicit_randn(shape=shape1,dtype='float64')
    # print(params)

    # for i in range(100):
    #     params,val = grad_descent(params, i)
    #     # if(val<-2):
    #     #   break
    ...
    f_scipy = tc.interfaces.scipy_interface(
        objective_fn, shape=shape2, jit=True)
    params = K.implicit_randn(shape=shape2)
    r = optimize.minimize(f_scipy, params, method="L-BFGS-B",
                          callback=callback, jac=True)
    print(r)
    best_theta = r.x
    ...
    # display the amplified state
    result = quantum_circuit_no_HT_return_state(
        theta=best_theta, num_of_qubits=num_of_qubits, start_state=initial_state_phi1)
    # print(result)
    # state_last = result.get_statevector().tolist()[4]
    state_last = result[-1]
    prb_last = np.linalg.norm(state_last)
